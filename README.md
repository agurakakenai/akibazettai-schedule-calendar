...
`typicalHeadcount` の累積がその日の人数に届くまで店舗を足す、という方式で**昼84.4% / 夜91.8%**の的中です。`typicalHeadcount` は見習い込みの人数なので、カレンダーに出る在籍だけの人数と比べると2割ほど余裕があり、それが「見習いが何人来るか分からない」ぶんの緩衝として働きます。ここにさらに見習いぶんを足すと過大評価になり、精度が落ちます（84.4%→82.5%）。
...
なお、店舗数の判定に使う定員は最頻値ではなく**平均**（`typicalHeadcount`）です。実測で比較したところ平均がいちばん当たりました（昼86.4% / 夜87.5%。中央値は昼86.4% / 夜83.5%、最頻値は昼84.1% / 夜83.5%）。
